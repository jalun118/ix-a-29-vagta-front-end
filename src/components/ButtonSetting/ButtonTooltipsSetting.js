...
export default function ButtonTooltipsSetting({ OnOpen, IsOffSet, ...props }) {
  ...
  return (
    <button onClick={() => ToggleTooltips()} {...props} className={`tooltip text-white justify-center items-center font-medium rounded-full text-sm p-2 outline-none shadow-xl shadow-gray-700/50 transition-all duration-300 delay-150 ease-in-out fixed ${OnOpen ? IsOffSet ? "bg-fuchsia-500 dark:bg-fuchsia-600 bottom-[12.6rem] right-[5rem] scale-1 z-0 visible hover:scale-[1.1] active:scale-[1.1]" : "bg-lime-500 dark:bg-lime-600 bottom-[12.6rem] right-6 scale-1 z-0 visible hover:scale-[1.1] active:scale-[1.1]" : "bottom-[5.5rem] right-6 scale-0 -z-40 invisible"} `}>
